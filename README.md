...
- **Gaming Mode** waits for startup activity, safely shuts down WSL, stops allowlisted services to a confirmed `STOPPED` state before terminating their remaining processes, and requires a 15-second continuously clean verification window before success. This prevents delayed service recovery and packaged-app respawns from slipping past a momentary clean check. It selects AMD Ryzen High Performance (`9935e61f-1661-40c5-ae2f-8495027d5d5d`) when present. Chrome and NVIDIA Overlay/Highlights remain available. OpenRGB and RustDesk remain required cleanup targets; Windows DCOM-owned WidgetService is intentionally best-effort and excluded from failure verification. A residual `wslservice` host process is benign only when WSLService, vmcompute, and `vmmemWSL` are stopped.
